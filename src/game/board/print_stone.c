...
static const char	*extra_char(const char *name, int is_rosette);
...
void	print_stone(const t_stone *s, int player_id, int col_num)
{
	if (!s)
		...
		printf("|  [%s] %s", s->name, extra_char(s->name, 0));
	else
		printf("|   %s  %s", s->name, extra_char(s->name, 0));
	if (col_num == 3)
		printf("|");
...
void	print_stone_rosette(const t_stone *s, int player_id, int col_num)
{
	if (!s)
		...
		printf("|▒▒[%s]▒%s", s->name, extra_char(s->name, 1));
	else
		printf("|▒▒ %s ▒%s", s->name, extra_char(s->name, 1));
	if (col_num == 3)
		printf("|");
